...
有一次我看 Anthropic 的账单，发现比预期高了很多。

排查后发现：agent 在做上下文压缩时，用的是 Claude Opus——和主任务用的同一个模型。压缩任务很简单，就是"把这段对话总结一下"，但我用了最贵的模型来做。

这是一个典型的"用大炮打蚊子"问题。不同的任务对模型能力的要求完全不同：
- 复杂的架构分析：需要 Claude Opus，推理能力强
- 简单的摘要生成：Haiku 就够了，便宜 20 倍
- 本地开发调试：Ollama 本地模型，免费且不需要网络

如果 agent 直接调用 `anthropic.messages.create()`，切换模型就要改业务代码。这违反了"开闭原则"——对扩展开放，对修改关闭。

解法是**适配器模式**：定义一个统一的接口，把差异封装在适配器里。业务代码只依赖接口，不依赖具体实现。
...
这里再啰嗦一下：适配器模式让切换模型变得容易，但不能保证切换后效果相同。在切换模型前，需要在你的具体任务上测试效果。"接口兼容"不等于"行为等价"——这是使用适配器模式时最容易被忽视的陷阱。
