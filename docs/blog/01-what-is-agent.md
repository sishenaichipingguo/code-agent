...
## 一、第一次跑 agent，我以为它会自己停

第一次写 agent 的时候，我以为它会像普通 API 调用一样：发一个请求，等一个响应，结束。

结果它一直在跑。

终端里不断滚动：读文件、搜索代码、再读文件、执行命令、再搜索……我盯着屏幕，不知道它在干什么，也不知道它什么时候会停。大概两分钟后，它输出了一段文字，然后停了。

那段文字是正确答案。

但我完全不知道它是怎么得到的。

这就是 agent 和普通 LLM 调用的本质区别：**普通 LLM 调用是一次性的，agent 是一个循环。**

## 二、为什么需要循环

LLM 本质上是一个文本转换函数：输入文本，输出文本。它没有手，没有眼，不能打开文件，不能运行命令。

早期让模型"行动"的方式是 prompt 解析：让模型输出特定格式的文本，然后用脚本解析执行：
...
这能用，但极其脆弱。模型输出格式稍有变化，解析就崩了。

2023 年 OpenAI 引入 Function Calling（Anthropic 叫 Tool Use），把这个流程标准化：模型不再输出自由文本，而是输出结构化的工具调用请求，格式由 API 保证。

但这只解决了"如何调用工具"的问题，没有解决"如何完成复杂任务"的问题。

复杂任务需要多步骤：读文件 → 分析内容 → 搜索相关代码 → 综合判断 → 给出答案。每一步的结果都影响下一步的决策。这不是一次 API 调用能完成的，需要一个循环。

**这就是 agent 的核心：把模型的输出作为下一轮的输入，反复迭代，直到任务完成。**

## 三、ReAct：循环的理论基础

2022 年 Google 的论文《ReAct: Synergizing Reasoning and Acting in Language Models》给这个循环起了个名字：让模型交替进行**推理（Reason）**和**行动（Act）**。
...
如果模型判断失误（比如认为任务完成了但其实没有，或者陷入了"调用工具 → 结果不满意 → 再调用 → 还是不满意"的死循环），循环就会出问题。
...
这里再啰嗦一下：`while (true)` 不是写 bug，是有意为之。循环的退出条件由模型的响应类型决定，不是由外部条件决定。这是 ReAct 模式的核心——模型自己决定什么时候停，不是外部代码决定的。
