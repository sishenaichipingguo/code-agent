...
LLM 本质上是一个文本转换函数：输入文本，输出文本。它没有手，没有眼，不能打开文件，不能运行命令，不能访问网络。
...
这能用，但极其脆弱。模型输出格式稍有变化（多个空格、换行位置不同、用了中文标点），解析就崩了。而且模型不知道"这段文本会被解析执行"，它只是在生成看起来合理的文本，没有任何约束。

2023 年 OpenAI 引入 **Function Calling**（后来 Anthropic 叫 Tool Use），把这个流程标准化：
...
工具系统的核心矛盾是：**工具越强大，agent 能做的事越多，但出错的代价也越高。**

一个只有 `read_file` 工具的 agent，最坏情况是读了不该读的文件。一个有 `bash` 工具的 agent，最坏情况是执行了 `rm -rf /`。

这个矛盾有几个维度：
- **工具粒度**：粗粒度工具（`bash`）灵活但危险，细粒度工具（`read_file`、`write_file`）安全但受限
- **工具数量**：工具越多，模型选择越困难，容易选错；工具太少，很多任务做不了
- **工具描述**：描述越详细，模型用得越准确，但 token 成本也越高
...
export async function createToolRegistry(): Promise<ToolRegistry> {
  ...
  // task, memory, plan, agent, send_message
