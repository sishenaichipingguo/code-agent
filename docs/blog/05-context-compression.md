...
## 二、核心矛盾：记忆完整性 vs 成本效率

上下文压缩的核心矛盾是：**你希望 agent 记住尽可能多的历史，但保留历史的成本随长度线性增长，而且超过上限时任务直接失败。**

这个矛盾有几个维度：

**压缩时机**：太早压缩会丢失还有用的信息；太晚压缩，API 会直接返回"prompt too long"错误，任务失败。你需要在"保留更多历史"和"避免溢出"之间找到平衡点。

**压缩质量**：摘要是有损压缩，总会丢失一些信息。关键是丢失的是"不重要的细节"还是"关键的上下文"。这很难保证——模型生成摘要时，它自己也不知道哪些信息在未来会用到。

**压缩成本**：生成摘要本身也需要调用模型，有额外的 token 成本和延迟。压缩太频繁，压缩本身的成本可能超过节省的成本。

**不可预测的突发**：有时候一个工具调用会返回巨大的结果（比如读取一个几万行的文件），context 可能在一轮之内从 70% 跳到 110%，主动检查来不及。

## 三、设计空间：如何处理 Context 溢出

面对 context 溢出问题，有几种不同的设计思路：

**方案 A：滑动窗口（Sliding Window）**
只保留最近 N 轮对话，丢弃更早的内容。实现简单，但会硬性丢失历史——如果关键信息在早期对话里，agent 会完全忘记它。

**方案 B：摘要压缩（Summarization）**
用模型生成历史对话的摘要，用摘要替换原始消息。保留了语义，但摘要是有损的，细节会丢失。这是目前最主流的方案。

**方案 C：外部记忆（External Memory）**
把重要信息存到外部存储（文件、数据库），需要时再加载。这是记忆系统（第 6 篇）的思路，和上下文压缩互补。

**方案 D：分层 Context（Hierarchical Context）**
把 context 分成"核心层"（始终保留）和"详细层"（按需加载）。实现复杂，但效果好。

这个项目选择了**方案 B（摘要压缩）+ 方案 C（外部记忆）的组合**：上下文压缩处理当前会话的短期记忆，记忆系统处理跨会话的长期记忆。
...
**ptlRetry 的单次重试**：`ptlRetry` 只重试一次，不再捕获第二次的 PTL 错误。这是有意为之——如果压缩后还是太长，说明有更根本的问题（比如单个工具结果就超过了 context window），需要在工具层面解决，而不是无限压缩。
