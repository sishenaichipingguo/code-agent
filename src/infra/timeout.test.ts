...
describe('executeWithTimeout', () => {
  ...
  it('rejects with default error when timeout exceeded', async () => {
    ...
    await expect(executeWithTimeout(slow, 50)).rejects.toThrow(
      'Timeout after 50ms'
    )
  })
  ...
  it('rejects with custom error when timeout exceeded', async () => {
    ...
    await expect(executeWithTimeout(slow, 50, customError)).rejects.toThrow(
      'custom timeout'
    )
  })
  ...
  it('propagates rejection from the original promise', async () => {
    ...
    await expect(executeWithTimeout(failing, 1000)).rejects.toThrow(
      'original error'
    )
  })
