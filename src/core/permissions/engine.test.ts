...
function makeTool(
  result: ReturnType<PermissionCapable['checkPermissions']>
): PermissionCapable {
  return {
...
describe('decide', () => {
  ...
  it('tool deny is hard block — cannot be overridden by rules', () => {
    ...
    expect(result).toEqual({
      type: 'deny',
      reason: 'rm always blocked in auto',
    })
  })
  ...
  it('matching allow rule returns allow', () => {
    const rule: AllowRule = {
      tool: 'bash',
      matcher: { kind: 'bash-prefix', prefix: 'git' },
      persistent: false,
    }
