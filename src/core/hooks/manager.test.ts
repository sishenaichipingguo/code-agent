...
describe('HookManager', () => {
  describe('fire()', () => {
    ...
    it('throws when onError is abort and command fails', async () => {
      ...
      const mgr = new HookManager(config)
      await expect(
        mgr.fire('pre-tool', { AGENT_TOOL_NAME: 'bash' })
      ).rejects.toThrow()
    })
    ...
    it('passes environment variables to the command', async () => {
      const config: HooksConfig = {
        'post-tool': [
          {
            command: 'bash -c "test \\"$AGENT_TOOL_NAME\\" = mybash"',
            onError: 'abort',
            timeout: 3000,
          },
        ],
      ...
      const mgr = new HookManager(config)
      await expect(
        mgr.fire('post-tool', { AGENT_TOOL_NAME: 'mybash' })
  ...
  describe('transform()', () => {
    ...
    it('returns modified payload from stdout JSON', async () => {
      ...
      const config: HooksConfig = {
        'pre-compress': [
          {
            command: `bash -c "echo '${json.replace(/"/g, '\\"')}'"`,
            onError: 'warn',
            timeout: 3000,
          },
        ],
